...
class BinaryTree {
    ...
    func lca(_ node: Node?, _ n1: Int, _ n2: Int) -> Node? {
        ...
        // Of both n1 and n2 are smaller than rooth, then LCA lies in Left
        if node!.data > n1 && node!.data > n2 {
            return lca(node?.left, n1, n2)
        }
        
        // If both n1 and n2 are large than root, then LCA lies in right
        if node!.data < n1 && node!.data < n2 {
            return lca(node?.right, n1, n2)
        }
        
        // Else we are at he node which is the LCA
        return node
    }
}
